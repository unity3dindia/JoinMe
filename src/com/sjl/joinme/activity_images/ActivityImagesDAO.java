...
public class ActivityImagesDAO {
	...
	public boolean addActivityImages(ActivityImagesDTO DTO) {
		...
		try {
			String query = "insert into activity_images( activity_id, description, image, created_datetime) values (?,?,?,?)";
	...
	public boolean deleteActivityImage(int image_id) {
		...
		try {
			String query = "delete from activity_image where image_id=?";
			ps = conn.prepareStatement(query);
	...
	public boolean editActivityImage(int image_id, ActivityImagesDTO dto) {
		...
		try {
			String query="update activity_images set activity_id=?,description=?,image=?,created_datetime=? where image_id=?";
			ps=conn.prepareStatement(query);
			ps.setInt(1, dto.getActivity_id());
			ps.setString(2, dto.getDescription());
			...
			System.out.println("+++Execution at editActivityImage");
		} finally {
			ps = null;
			conn = null;
			return flag;
